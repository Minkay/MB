...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insertaServicio = $servicioModel->insertaServicio($objServicioBean);

    header('Content-type: application/json');
    if (http_response_code() >= 200 && http_response_code() < 300) {
        if ($insertaServicio == true) {
            $insertaServ = array("status" => 1, "mensaje" => "Servicio Insertado Correctamente");
        } else {
            $insertaServ = array("status" => 0, "mensaje" => "Error al insertar Revise sus datos");
        }

        echo json_encode($insertaServ);
    } else {
        $insertaServ = array("status" => -1, "mensaje" => "Error en el llamado al servicio");
        echo json_encode($insertaServ);
    }
}
?>