...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insertaCosto = $costoModel->insertaCosto($objCostoBean);

    header('Content-type: application/json');
    if (http_response_code() >= 200 && http_response_code() < 300) {
        if ($insertaCosto == true) {
            $insertaCost = array("status" => 1, "mensaje" => "Costo Insertado Correctamente");
        } else {
            $insertaCost = array("status" => 0, "mensaje" => "Error al insertar Revise sus datos");
        }

        echo json_encode($insertaCost);
    } else {
        $insertaCost = array("status" => -1, "mensaje" => "Error en el llamado al servicio");
        echo json_encode($insertaCost);
    }
}
?>